...
import { VaccinationSiteCreate } from '../../pages/Admin/types';
import { getChildArr } from '../../pages/User/functions';
import { DistrictType, ProvinceType, WardType } from '../../pages/User/types';
import { useCreateVaccinationSite } from '../../hooks/useVaccinationSite';
interface Props {
  onClose: () => void;
  handleRefetch: () => void;
}
export const ContentDialog = (props: Props) => {
  const { createVaccinationSite } = useCreateVaccinationSite();
  ...
  const {
    ...
    defaultValues: {
      ...
      site_manager: undefined,
      number_of_vaccination_table: undefined
    }
  });
  ...
  const formSubmitHandler: SubmitHandler<VaccinationSiteCreate> = async (
    ...
    setLoading(true);
    try {
      const response = await createVaccinationSite(data);
